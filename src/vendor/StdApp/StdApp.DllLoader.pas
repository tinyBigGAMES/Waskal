{===============================================================================
 ...
 -------------------------------------------------------------------------------
  ...
  StdApp.DllLoader - In-memory PE/DLL loader

  Loads DLL images directly from memory or embedded resources without
  writing to disk. Hooks the Windows loader (ntdll) to map a PE image
  from a memory buffer as if it were a real file, producing a fully
  functional module handle with resolved imports and relocations.

  Key routines:
  - LoadLibrary(AData, ASize): Load a DLL from a raw memory pointer
  - RegisterDllData(AModuleName, AData, ASize): Register data for
    deferred loading by name
  - RegisterDllData(AModuleName, AResName): Register from RT_RCDATA
    resource
  - LoadLibrary(AModuleName): Load a previously registered DLL by name
  - LoadAll(): Load all registered DLLs

  Dependencies: none (WinApi and System units only)
  Notes: Uses NT Native API hooking internally. Compatible with
    Windows 10/11 including 24H2. Supports LOAD_FLAGS for header
    erasure, PEB unlisting, and thread-call suppression.
===============================================================================}

unit StdApp.DllLoader;
...
// Removes the persistent LDR hooks after a loading operation completes.
// Hooks are re-installed automatically by EnsureLdrGetDllHandleHooked on the
// next load call. This prevents race conditions with multi-threaded DLL loading
// (e.g. GPU drivers spawning threads during window creation).
procedure UnhookLdr();
begin
  ...
  if GLdrLoadDllHookInstalled and (GLdrLoadDllAddr <> nil) then
  begin
    EnterCriticalSection(GRedirectCritSect);
    try
      UninstallHook(GLdrLoadDllAddr, GOrigBytesLdrLoadDll);
      GLdrLoadDllHookInstalled := False;
    finally
      LeaveCriticalSection(GRedirectCritSect);
    end;
  end;
...
begin
  ...
  if LPath = '' then
  begin
    OutputDebugStringFmt('Myra.DllLoader.LoadLibrary Error: Could not find path to dummy DLL "advapi32res.dll". Cannot proceed.', []);
    SetLastError(ERROR_FILE_NOT_FOUND); // Indicate failure to find necessary file.
    Result := 0;
    Exit;
  end;
  OutputDebugStringFmt('Myra.DllLoader.LoadLibrary: Using dummy path "%s".', [LPath]);
  ...
  // Remove LDR hooks — they'll be re-installed on the next load call.
  UnhookLdr();
end;
  ...
  begin
    ...
    begin
      ...
      if not PatchModuleName(Result, AModuleName) then
        OutputDebugStringFmt('Myra.DllLoader.LoadLibrary: Warning - Failed to patch name to "%s".', [AModuleName]);
    end;
  end;
...
initialization
  ...
  OutputDebugStringFmt('Myra.DllLoader: Initialized critical section.', []);
  // Optionally resolve NT function pointers here if preferred over lazy initialization.
  // InitializeNtFunctions();

finalization
  // Uninstall any remaining LDR hooks (both LdrGetDllHandle and LdrLoadDll).
  UnhookLdr();
  ...
  OutputDebugStringFmt('Myra.DllLoader: Finalized (deleted critical section).', []);
